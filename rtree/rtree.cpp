...
// Búsqueda pública
void RTree::Search(const Rect& searchRect, vector<vector<pair<int, int>>>& results) {
	results.clear();
	if (m_root != nullptr) {
		SearchRec(m_root, searchRect, results);
	}
}

// Búsqueda recursiva
void RTree::SearchRec(Node* a_node, const Rect& searchRect, vector<vector<pair<int, int>>>& results) {
	if (a_node->IsLeaf()) {
		// Nodo hoja → contiene datos
		for (int i = 0; i < a_node->m_count; ++i) {
			Branch& branch = a_node->m_branch[i];
			if (Overlap(&branch.m_rect, const_cast<Rect*>(&searchRect))) {
				results.push_back(branch.m_data); // Guardamos los puntos
			}
		}
	}
	else {
		...
		for (int i = 0; i < a_node->m_count; ++i) {
			Branch& branch = a_node->m_branch[i];
			if (Overlap(&branch.m_rect, const_cast<Rect*>(&searchRect))) {
				SearchRec(branch.m_child, searchRect, results);
			}
		}
	}
}
